...
def listen_latest_messages_q():
        try:
            ...
            if msgs:
                ...
                if msg_id not in processed_msg_ids:
                    # 将self_name改为列表形式
                    names_to_check = self_name  # 这里添加所有需要检查的用户名
                    
                    # 检查是否被@或包含任一用户名
                    if '�' in latest_msg[1] or any(name in latest_msg[1] for name in names_to_check):
                        print(f"{get_time()} 你被@了！")
                        processed_msg_ids.add(msg_id)
                        # 去除所有可能的@用户名内容
                        cleaned_msg = latest_msg[1]
                        for name in names_to_check:
                            cleaned_msg = cleaned_msg.replace(name, '').strip()
                        return latest_msg[0], cleaned_msg
                    else:
                        c = c_true.up_message(b, latest_msg[1], self_name)  # 调用判断函数
                        if c[0] == 'true':  # 如果判断结果为true
                            print(f"{get_time()} 被判断为呼唤")
                            print('\n\n\n\n\n' + str(c))  # 确保转换为字符串
                            processed_msg_ids.add(msg_id)
                            return latest_msg[0], latest_msg[1]  # 直接返回原始消息，不进行清理
                        else:  # 如果判断结果为false
                            time.sleep(0)
                            return None  # 返回None表示没有被@
        except Exception as e:
            print(f"{get_time()}监听出错: {str(e)}")
            time.sleep(0)
